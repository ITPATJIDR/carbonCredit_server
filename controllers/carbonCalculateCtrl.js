		...
		try{
			...
			await connection.query(getUser_sql, getUser_data, async (err, result) => {
				if(err){
					...
					const new_growth_a_tree = ((Number(compensate_CC) + Number(offset)) / 12).toFixed(0)
					...
					await connection.query(getCCbank_sql, async (err , ccBank) => {
						...
						const new_growth_a_tree_main = ((Number(compensate_CC_main) + Number(offset)) / 12).toFixed(0)
